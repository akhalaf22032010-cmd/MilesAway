...
function validateCommands(commands) {
    ...
    for (const cmd of commands) {
        ...
        for (const option of cmd.options) {
            ...
            if (option.description && option.description.length > 110) validationErrors.push(`Command ${cmd.name} option ${option.name} has description longer than 110 chars: "${option.name}" (${option.description.length} chars)`);
            if (option.choices) {
                for (const choice of option.choices) {
                    if (choice.name && choice.name.length > 110) validationErrors.push(`Command ${cmd.name} option ${option.name} choice ${choice.name} has name longer than 110 chars: "${choice.name}" (${choice.name.length} chars)`);
                    if (choice.value && choice.value.length > 100) validationErrors.push(`Command ${cmd.name} option ${option.name} choice ${choice.name} has value longer than 100 chars: "${choice.name}" (${choice.name.length} chars)`);
                }
            }
            ...
            for (const subOption of option.options) {
                ...
                if (subOption.description && subOption.description.length > 110) validationErrors.push(`Command ${cmd.name} subcommand ${option.name} option ${subOption.name} has description longer than 110 chars: "${subOption.name}" (${subOption.name.length} chars)`);
            }
        }
    }
...
export async function registerCommands(client, options = {}) {
    try {
        ...
        if (!guilds.length) throw new Error('Bot is not currently in any guilds');
